...
parser.add_argument( "--learning-rule", dest='rule', default="oJa",choices=[ "oJa", "STDP" ,"BCM"],
                     help= 'Set the learning rule, default is oJa' )
parser.set_defaults( feature=True )
args = parser.parse_args()

# allocate folder to save outputs
dir_images, dir_name, dir_data = make_timestamped_dir( root=args.plots_directory )
...
with model:
    ...
    if args.rule == 'oJa':
        conn = nengo.Connection(pre.neurons, post.neurons,
                                learning_rule_type=
                                nengo.learning_rules.Oja(learning_rate=args.learning_rate),
    ...
    if args.rule == 'BCM':
        conn = nengo.Connection(pre.neurons, post.neurons,
                                learning_rule_type=
                                nengo.learning_rules.BCM(learning_rate=args.learning_rate),
                                # transform=np.random.random( (post.n_neurons, pre.n_neurons) )
                                transform=np.random.normal(0.5, 0.25, (post.n_neurons, pre.n_neurons))
                                )
    if args.rule == 'STDP':
        conn = nengo.Connection( pre.neurons, post.neurons,
                                 learning_rule_type=TripletSTDP(
